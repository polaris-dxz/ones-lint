 ...
 * @param cwd
 * @param pkg
 * @returns eslint-config-ones/index
 * @returns eslint-config-ones/react
 * @returns eslint-config-ones/typescript/index
 * @returns eslint-config-ones/typescript/react
 */
const getConfigType = (cwd: string, pkg: PKG): string => {
  ...
  return 'eslint-config-ones/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '');
};
